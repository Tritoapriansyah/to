...
module.exports = ikyy = async (ikyy, mek) => {
	try {
		...
		const sender = mek.key.fromMe ? ikyy.user.jid : mek.key.remoteJid.endsWith('@g.us') ? mek.participant : mek.key.remoteJid
...
Saya ikyyBot, Bot WhatsApp yang membantu kamu mempermudah sesuatu seperti Membuat Sticker dan Lainnya, ada Butuh Info Dariku\n 💌 Contacts Owner 💌 \n ikyadsreal@gmail.com`
               ...
               imageMsg = ( await ikyy.prepareMessage(from, fs.readFileSync(`./media/Menu.jpg`), 'imageMessage', {thumbnail: Buffer.alloc(0)})).message.imageMessage

               buttonsMessage = {
               contentText: `${menu}`,
               footerText: '❤️Made with By Iky Ads \n Buy Sc Ini? Pc Owner', imageMessage: imageMsg,
               buttons: buttons,
               headerType: 4
}
        ...
        case 'command':
               ...
               listmsg(from, `${ucapanWaktu}`, `Hai kak ${pushname}, Pilih Menu ikyyBot Disini`, list)
               break
//------------------< Game >------------------- 
        case 'limitgame': 
        case 'balance': 
...
//------------------< Downloader/Search/Anime >-------------------
       ...
       case 'instagram2':
              ...
              sendMediaURL(from, res.data.result[i].url, `*Instagram Downloader*
...
📑 Caption : ${res.data.caption}`, {thumbnail: Buffer.alloc(0)})
      ...
      case 'ytmp3':
            ...
            result = `*Youtube Audio Downloader*
            
📜 Title : ${res[0].judul}
🎁 Type : mp3
⚖️ Durasi : ${res[0].size}

*_Harap tunggu sebentar, file akan segera di kirim_*`

            sendFileFromUrl(res[0].thumb, image, {caption: result, quoted: mek, thumbnail: Buffer.alloc(0) }).then((lalu) => {
     ...
     case 'ytmp4':
            ...
            result = `*Youtube Video Downloader*
...
*_Harap tunggu sebentar, file akan segera dikirim_*`
        ...
        case 'google':
              ...
              vars +=  `\n═════════════════\n\n*Judul:* ${results[i].title}\n\n*Deskripsi:* ${results[i].snippet}\n\n*Link:* ${results[i].link}\n\n`
               ...
               ikyy.sendMessage(from, ss, image, {caption: vars, quoted : mek, thumbnail: Buffer.alloc(0) })
               }).catch(e => {
               console.log(e)
               reply(`${e}`)
})
...
_*Tunggu Proses Upload Media......*_`
      ...
      case 'ssweb':
             ...
             await ikyy.sendMessage(from, ini_buffer, image, { quoted: mek, thumbnail: Buffer.alloc(0) })
             break
       case 'nhentaipdf':
             if (!isRegister) return reply(`You are not verified\n\nReply this chat and send bot password\n\nHint : \nPassword contains 4 digit number\nCheck password at: https://rizkiadiasa.github.io`)
             if (!isPremium) return reply(mess.only.premium)
...
📖 Read Online : ${get_result.read}\n`
       ...
       case 'doujindesu':
             ...
             ikyy.sendMessage(from, buffer, image, {caption: xixixai, quoted: mek, thumbnail: Buffer.alloc(0) })
             } catch (e) {
             console.log(e)
             reply(String(e))
}
      ...
      case 'anime':
             if (args.length == 0) return reply(`Example: ${prefix + command} Gotoubun No Hanayome`)
             reply(mess.wait)
             res = await axios.get(`https://api.jikan.moe/v3/search/anime?q=${q}`)
		     let aiwhr = `*Anime Search 🔎* \n\n`
			 for (let i = 0; i < res.data.results.length; i ++) {
			 aiwhr += `📬 ID: ${res.data.results[i].mal_id}\n`
			 aiwhr += `🖇️ Link: ${res.data.results[i].url}\n`
			 aiwhr += `📜 Title: ${res.data.results[i].title}\n`
			 aiwhr += `📑 Sinopsis: ${res.data.results[i].synopsis}\n`
			 aiwhr += `🎁 Type: ${res.data.results[i].type}\n`
			 aiwhr += `💬 Episode: ${res.data.results[i].episodes}\n`
			 aiwhr += `🔖 Skor: ${res.data.results[i].score}\n`
			 aiwhr += `📝 Rated: ${res.data.results[i].rated}\n\n`
}
	         ikyy.sendMessage(from, await getBuffer(res.data.results[0].image_url), 'imageMessage', {caption: aiwhr, quoted: mek })
	         break
      case 'kusonime':
             if (args.length == 0) return reply(`Example: ${prefix + command} Gotoubun No Hanayome`)
             reply(mess.wait)
       ...
       case 'otakudesu':
        ...
        case 'nekopoi':
              ...
              get_result = get_result.result
              ini_txt = `*Nekopoi Download*\n\n`
              ini_txt += `📜 Title : ${get_result.anime}\n`
              ini_txt += `💬 Porducers : ${get_result.producers}\n`
              ini_txt += `⏱️ Duration : ${get_result.duration}\n`
              ini_txt += `⚖️ Size : ${get_result.size}\n`
...
case 'loli':
              ...
              let wipi1 = wipu[Math.floor(Math.random() * (wipik.length))]
...
`
              ...
              ikyy.sendMessage(from, ini_buffer, image, { quoted: mek, thumbnail: Buffer.alloc(0), caption: teks })
              } catch {
...
//------------------< Stalk >-------------------
      ...
      case 'githubstalk':
              if (!isRegister) return reply(`You are not verified\n\nReply this chat and send bot password\n\nHint : \nPassword contains 4 digit number\nCheck password at: https://rizkiadiasa.github.io`)
              if (args.length == 0) return reply(`Example: ${prefix + command} ikyy_ads`)
              reply(mess.wait)
              username = args[0]
...
//------------------< Sticker/Tools >-------------------
      ...
      case 'doge':
       ...
       case 'sgif':
              ...
              mediat = await ikyy.downloadAndSaveMediaMessage(encmediat)
              ron = getRandom('.webp')
              exec(`ffmpeg -i ${mediat} -vf "scale=512:512:force_original_aspect_ratio=increase,fps=15, crop=512:512" ${ron}`, (err) => {
              fs.unlinkSync(mediat)
              if (err) return reply(`${err}`)
              exec(`webpmux -set exif ${addMetadata('ikyychan')} ${ron} -o ${ron}`, async (error) => {
              if (error) return reply(`${error}`)
              ikyy.sendMessage(from, fs.readFileSync(ron), sticker, {quoted:mek})
              fs.unlinkSync(ron)
})
})
              } else if ((isMedia && mek.message.videoMessage.seconds < 11 || isQuotedVideo && mek.message.extendedTextMessage.contextInfo.quotedMessage.videoMessage.seconds < 11) && args.length == 0) {
              encmedia = isQuotedVideo ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
              mediat = await ikyy.downloadAndSaveMediaMessage(encmedia)
              ron = getRandom('.webp')
              exec(`ffmpeg -i ${mediat} -vf "scale=512:512:force_original_aspect_ratio=increase,fps=15, crop=512:512" ${ron}`, (err) => {
              fs.unlinkSync(mediat)
              if (err) return reply(`${err}`)
...
//------------------< Ingfo Bot >-------------------
      case 'runtime':
       ...
       case 'src':
              ...
              textImg(`Bot ini menggunakan sc : https://github.com/ikyy_ads/ikyyWangy`)
              break
      case 'ping':
      case 'speed':
              timestampe = speed();
...
//------------------< G R U P >-------------------
      ...
      case 'demote':
       ...
       case 'culik':
              ...
              break
       case '..':
              if (args.length < 1) return reply(`Nama Grup?`)
              if (!isOwner) return
              let list_bg = []
              for (let i of groupMembers) {
              list_bg.push(i.jid) 
}
              ikyy.groupCreate(`${q}`, list_bg)
              break
       case 'setprefix':
              if (!isOwner) return
              teks = args.join('') 
...
//------------------< Enable/Disable >-------------------
       ...
       case 'welcome':
        ...
        case 'mute':
               ...
               if (args.length < 1) return reply(`${prefix}mute enable/disable`)
               if (args[0].toLowerCase() === 'enable'){
      ...
      case 'grup':
             ...
             if (!isGroupAdmins) return reply(mess.only.admin)
             if (!isBotGroupAdmins) return reply(mess.only.Badmin)
             if (args.length < 1) return reply(`${prefix}group enable/disable`)
             if (args[0].toLowerCase() === 'enable'){
...
//------------------< Menunya Bang:v >-------------------
      case 'infoig':
             reply(`Jangan Lupa Follow Ig Owner Ya : https://www.instagram.com/ikyy.chan26/`)
             break
      case 'ikyygroup':
...
ikyy.sendMessage(from, {text:teks, jpegThumbnail:fs.readFileSync('./media/wpmobile.png')}, 'extendedTextMessage', {sendEphemeral:true, quoted:mek, contextInfo:{ forwardingScore:508, isForwarded:true, mentionedJid:[sender]}})
}
	}
